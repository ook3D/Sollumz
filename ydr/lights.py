...
def export_light(light_obj: Object, parent_obj: Object) -> Light:
    ...
    cone_outer_angle = min(max(degrees(light_props.cone_outer_angle), 0.0), 90.0) if is_spot else 0.0
    cone_inner_angle = min(max(degrees(light_props.cone_inner_angle), 0.0), cone_outer_angle) if is_spot else 0.0
